...
pseudoSEmatrixSR1=np.zeros(shape=(len(chilist),len(philist)))
scalarSEmatrixSR2=np.zeros(shape=(len(chilist),len(philist)))
pseudoSEmatrixSR2=np.zeros(shape=(len(chilist),len(philist)))
...
for med in ['scalar','pseudo']:
    for ichi in range(len(chilist)):
        for iphi in range(len(philist)):
            filename=os.path.join(dirpath,"Output_"+med+"_NLO_Mchi-"+str(chilist[ichi])+"_Mphi-"+str(philist[iphi])+".root")
...
print pseudoSEmatrixSR1     
print pseudoSEmatrixSR2
for med in ['scalar','pseudo']:
    for reg in ['1','2']:              

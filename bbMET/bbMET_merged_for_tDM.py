...
def AnalyzeDataSet():
    ...
    for ievent in range(NEntries):
        ...
        ####new conds
        jetcond=True
        if j1.Pt() < 30.0 and myJetNhadEF[ifirstjet] > 0.8 and myJetChadEF[ifirstjet]< 0.1: jetcond=False
        ...
        if nEle==1 and nMu==0 and nTauTightElectron==0 and EleCRtrigstatus and jetcond and Wenumass>=160. and pfMet >= 160.:
            iLeadLep=0
            barrel_ele = (abs(myEles[iLeadLep].Eta()) > 1.566) or (abs(myEles[iLeadLep].Eta()) < 1.4442)
            if myEles[iLeadLep].Pt() > 30. and myEleTightID[iLeadLep] and  abs(myEles[iLeadLep].Eta()) < 2.1 and barrel_ele:
        ...
        if nMu==1 and nEle==0 and nTauTightMuon==0 and MuCRtrigstatus and jetcond and Wmunumass>=160. and pfMet>=160.:
            iLeadLep=0

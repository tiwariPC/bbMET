...
#    st_disc_againstMuonMedium       =    ROOT.std.vector('bool')()
    ...
    st_LepCategory = ROOT.std.vector('int')()
    ...
    WtaunuRecoil = array( 'f', [ 0. ] )
    Wtaunumass = array( 'f', [ 0. ] )
    WtaunuPhi = array( 'f', [ 0. ] )
...
#    outTree.Branch( 'st_disc_againstMuonMedium', st_disc_againstMuonMedium)
    ...
    outTree.Branch( 'st_LepCategory',st_LepCategory )
    ...
    outTree.Branch( 'WtaunuRecoil', WtaunuRecoil, 'WtaunuRecoil/F')
    outTree.Branch( 'Wtaunumass', Wtaunumass, 'Wtaunumass/F')
    outTree.Branch( 'WtaunuPhi', WtaunuPhi, 'WtaunuPhi/F')
...
#        print st_pu_nTrueInt[0]
        ...
        numLepCat=ttbarDecayModeChecker(genParId,genMomParId,genDa1,genDa2)
#        print numLepCat
        st_LepCategory.push_back(numLepCat)
...
# ------------------
        ...
        if len(myTaus) == 1:
            tau1 = myTaus[0]
            p4p4_tau1 = tauP4[tau1]

            tau_mass = MT(p4p4_tau1.Pt(),pfMet, DeltaPhi(p4p4_tau1.Phi(),pfMetPhi)) #transverse mass defined as sqrt{2pT*MET*(1-cos(dphi)}

            WtaunuRecoilPx = -( pfMet*math.cos(pfMetPhi) + p4p4_tau1.Px())
            WtaunuRecoilPy = -( pfMet*math.sin(pfMetPhi) + p4p4_tau1.Py())
            WtaunuRecoilPt = math.sqrt(WtaunuRecoilPx**2  +  WtaunuRecoilPy**2)
            if WtaunuRecoilPt > 200.:
                WtaunuRecoil[0] = WtaunuRecoilPt
                Wtaunumass[0] = tau_mass
                WtaunuPhi[0] = arctan(WtaunuRecoilPx,WtaunuRecoilPy)
        ...
        if len(myEles) == 1:
            ...
            WRecoilstatus =(WtaunuRecoil[0] > 200)
        else:
            WRecoilstatus=False
...
# ------------------
        ...
        ## for Single electron && Single Tau
        if len(myEles)==1 and len(myTaus)==1 :
            ele1 = myEles[0]
            p4_ele1 = eleP4[ele1]
            tau1 = myTaus[0]
            p4p4_tau1 = tauP4[tau1]

            TOPenutaunuRecoilPx = -( pfMet*math.cos(pfMetPhi) + p4p4_tau1.Px() + p4_ele1.Px())
            TOPenutaunuRecoilPy = -( pfMet*math.sin(pfMetPhi) + p4p4_tau1.Py() + p4_ele1.Py())
            TOPenutaunuRecoilPt =  math.sqrt(TOPenutaunuRecoilPx**2 + TOPenutaunuRecoilPy**2)
            if TOPenutaunuRecoilPt > 200:
                TOPRecoil[0] = TOPenutaunuRecoilPt
                TOPPhi[0] = arctan(TOPenutaunuRecoilPx,TOPenutaunuRecoilPy)

        ## for Single tau && Single Muon
        if len(myTaus)==1 and len(myMuos)==1:
            tau1 = myTaus[0]
            p4p4_tau1 = tauP4[tau1]
            mu1 = myMuos[0]
            p4_mu1 = muP4[mu1]

            TOPtaunumunuRecoilPx = -( pfMet*math.cos(pfMetPhi) + p4_mu1.Px() + p4p4_tau1.Px())
            TOPtaunumunuRecoilPy = -( pfMet*math.sin(pfMetPhi) + p4_mu1.Py() + p4p4_tau1.Py())
            TOPtaunumunuRecoilPt =  math.sqrt(TOPtaunumunuRecoilPx**2 + TOPtaunumunuRecoilPy**2)
            if TOPtaunumunuRecoilPt > 200:
                TOPRecoil[0] = TOPtaunumunuRecoilPt
                TOPPhi[0] = arctan(TOPtaunumunuRecoilPx,TOPtaunumunuRecoilPy)
        #if ZRecoilstatus:
...
def ttbarDecayModeChecker(genParId,genMomParId,genDa1,genDa2):
    mymu=[]
    myel=[]
    mytau=[]
    W_hadCond=False
    ...
    for i in range(len(genParId)):
        ...
        if (abs(genParId[i])==24 and abs(genMomParId[i])==6 and (abs(genDa1[i])==11 or abs(genDa2[i])==11)):
            myel.append(1)
            W_eleCod=True
        elif (abs(genParId[i])==24 and abs(genMomParId[i])==6 and (abs(genDa1[i])==13 or abs(genDa2[i])==13)):
            mymu.append(2)
            W_muCond=True
        elif (abs(genParId[i])==24 and abs(genMomParId[i])==6 and (abs(genDa1[i])==15 or abs(genDa2[i])==15)):
            mytau.append(3)
            W_tauCond=True
        elif (abs(genParId[i])==24 and abs(genMomParId[i])==6) and not ( eleCond or muCon or TauCon):  W_hadCond=True

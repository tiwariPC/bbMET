...
def submitjob(jobcount,fname,tempfile):    
    ...
    submittemp.write("arguments = $(Proxy_filename) "+tempfile.split('/')[1]+" "+fname.split('.')[0]+" "+str(jobcount)+'\nqueue')
    ...
    if not test: os.system("CONDOR_SCHEDD_HOST=bigbird15.cern.ch CONDOR_CREDD_HOST=bigbird15.cern.ch condor_submit submit_multi_temp.sub")
    count+=1
